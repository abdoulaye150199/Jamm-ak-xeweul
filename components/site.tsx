...
export function ActivityCard({ activity, compact = false }: { activity: typeof activities[number]; compact?: boolean }) { return <article className={`group relative overflow-hidden rounded-[1.6rem] bg-brand-900 ${compact ? 'h-64 sm:h-72' : 'h-[300px] sm:h-[390px]'}`}><img src={activity.image} alt="" className="absolute inset-0 h-full w-full object-cover opacity-80 transition duration-700 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-brand-950 via-brand-950/35 to-transparent" /><div className="relative z-10 flex h-full flex-col justify-end p-5 text-white sm:p-6"><div className="mb-3 flex items-center justify-between gap-2"><span className="rounded-full bg-sun-500 px-3 py-1 text-[10px] font-extrabold uppercase tracking-wider text-brand-950">{activity.category}</span><span className="flex items-center gap-1 text-xs text-white/65"><CalendarDays size={12} /> {activity.photos}</span></div><h3 className={`${compact ? 'text-xl' : 'text-2xl'} font-extrabold leading-tight tracking-tight`}>{activity.title}</h3><p className="mt-3 text-xs font-bold uppercase tracking-wider text-white/55">{activity.date}</p></div></article>; }
