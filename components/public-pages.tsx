...
function ActivitiesPage() { return <div className="min-h-screen bg-[#f6faf7]"><PublicHeader /><main><section className="hero-grid px-5 py-20 text-white lg:py-24"><div className="mx-auto max-w-7xl"><p className="eyebrow text-sun-400">Sur le terrain</p><h1 className="mt-4 text-5xl font-extrabold">Nos activités.</h1><p className="mt-5 max-w-xl text-lg leading-8 text-white/65">Des rencontres, des actions concrètes et des moments pour faire vivre la solidarité dans nos quartiers.</p></div></section><section className="mx-auto max-w-7xl px-5 py-20"><div className="mb-10 flex items-end justify-between"><div><p className="eyebrow text-brand-600">Galerie</p><h2 className="mt-3 text-4xl font-extrabold text-brand-900">Les moments forts</h2></div><span className="hidden text-sm font-bold text-slate-400 sm:block">15 quartiers · 32 actions</span></div><div className="grid gap-5 md:grid-cols-3">{activities.map(activity => <ActivityCard key={activity.title} activity={activity} />)}</div></section></main><Footer /><ChatWidget /></div>; }
  ...
  return <div>
    <div className={`rounded-2xl border p-3 transition ${listening ? 'border-red-300 bg-red-50' : 'border-slate-200 bg-[#fbfcfb]'}`}>
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="hidden rounded-lg bg-slate-100 px-3 py-2 text-xs font-bold text-slate-500 sm:inline-flex">Ou écrivez ci-dessous</span>
      </div>
...
export function ContributionPage({ kind }: { kind: 'need' | 'idea' }) { const [submitted, setSubmitted] = useState(false); const [description, setDescription] = useState(''); const isNeed = kind === 'need'; function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setSubmitted(true); } return <div className="min-h-screen bg-[#f6faf7]"><PublicHeader /><main className="mx-auto grid max-w-7xl gap-12 px-5 py-16 lg:grid-cols-[.7fr_1.3fr] lg:py-24"><div><p className={`eyebrow ${isNeed ? 'text-red-500' : 'text-sun-600'}`}>{isNeed ? 'Remonter un besoin' : 'Boîte à idées'}</p><h1 className="mt-4 text-5xl font-extrabold leading-tight text-brand-900">{isNeed ? 'Votre quartier mérite toute notre attention.' : 'Les bonnes idées commencent par une voix.'}</h1><p className="mt-6 text-lg leading-8 text-slate-500">{isNeed ? 'Décrivez un problème observé dans votre quartier. Notre équipe le qualifiera et vous tiendra informé de son évolution.' : 'Partagez un projet ou une proposition pour améliorer la vie à Thiès-Nord. Chaque idée est étudiée par notre équipe programme.'}</p><div className="mt-9 space-y-4 text-sm font-bold text-brand-900"><p className="flex gap-3"><ShieldCheck size={19} className="shrink-0 text-brand-500" /> Vos informations restent confidentielles.</p><p className="flex gap-3"><CheckCircle2 size={19} className="shrink-0 text-brand-500" /> Un accusé de réception vous sera envoyé.</p></div></div><div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-soft sm:p-10"><div className="mb-8 flex items-center gap-4 border-b border-slate-100 pb-6"><div className={`rounded-2xl p-3 ${isNeed ? 'bg-red-50 text-red-500' : 'bg-sun-50 text-sun-600'}`}>{isNeed ? <FileText size={24} /> : <Lightbulb size={24} />}</div><div><h2 className="text-2xl font-extrabold text-brand-900">{isNeed ? 'Déclarer un besoin' : 'Proposer une idée'}</h2><p className="mt-1 text-sm text-slate-500">Les champs marqués * sont obligatoires.</p></div></div>{submitted ? <div className="rounded-2xl border border-brand-200 bg-brand-50 p-6 text-brand-700"><Check className="mb-3" /><h3 className="text-xl font-extrabold">Merci pour votre contribution.</h3><p className="mt-2 text-sm leading-6">Votre demande a bien été enregistrée. Vous pourrez suivre son évolution depuis votre espace citoyen.</p><Link href="/espace-membre" className="mt-5 inline-flex items-center gap-2 font-bold underline">Accéder à mon espace <ArrowRight size={15} /></Link></div> : <form onSubmit={submit} className="space-y-5"><div className="grid gap-4 sm:grid-cols-2"><label><span className="mb-2 block text-sm font-bold text-slate-700">Votre nom *</span><input required className="input-focus" placeholder="Awa Diop" /></label><label><span className="mb-2 block text-sm font-bold text-slate-700">Téléphone *</span><input required className="input-focus" placeholder="+221 77 000 00 00" /></label></div><label className="block"><span className="mb-2 block text-sm font-bold text-slate-700">Quartier *</span><select required className="input-focus"><option value="">Sélectionner un quartier</option><option>Nguinth</option><option>Diakhao</option><option>Keur Mame El Hadj</option></select></label><label className="block"><span className="mb-2 block text-sm font-bold text-slate-700">{isNeed ? 'Titre du besoin *' : 'Titre de votre idée *'}</span><input required className="input-focus" placeholder={isNeed ? 'Ex. Éclairage public rue 14' : 'Ex. Atelier de formation numérique'} /></label><div><span className="mb-2 block text-sm font-bold text-slate-700">{isNeed ? 'Décrivez le problème *' : 'Décrivez votre proposition *'}</span><VoiceDescription value={description} onChange={setDescription} /></div><label className="flex cursor-pointer items-center gap-3 rounded-2xl border border-dashed border-slate-300 px-4 py-3 text-sm font-bold text-slate-600 transition hover:border-brand-400 hover:bg-brand-50"><ImagePlus size={18} className="text-brand-500" /> Joindre une photo <span className="font-normal text-slate-400">(optionnel)</span><input type="file" accept="image/*" className="sr-only" /></label><label className="flex items-start gap-2 text-xs leading-5 text-slate-500"><input required type="checkbox" className="mt-1 h-4 w-4 rounded border-slate-300 text-brand-600" /> J’autorise JÀMM AK XÉEWAL à utiliser ces informations pour traiter ma contribution.</label><button className={`inline-flex w-full items-center justify-center gap-2 rounded-xl px-5 py-3.5 text-sm font-bold text-white shadow-lg transition hover:-translate-y-0.5 ${isNeed ? 'bg-red-500 shadow-red-500/20 hover:bg-red-600' : 'bg-brand-600 shadow-brand-600/20 hover:bg-brand-700'}`}>Envoyer ma contribution <Send size={16} /></button></form>}</div></main><Footer /></div>; }
